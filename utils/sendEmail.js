  ...
  <div style="font-family: sans-serif; line-height: 1.5; color: #333;">
    <div style="padding: 1rem; border: 1px solid #eee; border-radius: 8px; max-width: 600px; margin: auto;">
      <div style="text-align: center; margin-bottom: 1rem;">
        <h2 style="color: #270450;">Prompt Palace Community</h2>
      </div>
      ${content}
...
export const sendOTPEmail = async (to, subject, otp) => {
  const html = emailWrapper(`
    ...
    <h2 style="font-size: 2rem; text-align: center; color: #0057B7; margin: 1rem 0;">${otp}</h2>
    <p>This OTP will expire in 10 minutes.</p>
  `);

  try {
    const { data, error } = await resend.emails.send({
      from: 'Prompt Palace <onboarding@resend.dev>', 
      to,
      subject,
      html,
    });
...
export const sendForgotPasswordEmail = async (to, resetToken) => {
  ...
  const html = emailWrapper(`
    ...
    <p style="text-align: center;">
      <a href="${resetLink}" style="background-color:#0057B7; color:white; padding:12px 24px; text-decoration:none; border-radius:5px; display:inline-block;">Reset Password</a>
    </p>
...
// Keep other email functions but update them similarly to use Resend
